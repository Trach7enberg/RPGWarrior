...
#include "Controllers/AI/WarriorAIController.h"
...
AWarriorAIController::AWarriorAIController(const FObjectInitializer& ObjectInitializer):
	// 覆盖父类寻路组件,采用CrowdFollowingComponent解决拥挤问题
...
{
	const auto ToCheckActor = Cast<ACharacter>(&Other);
...
void AWarriorAIController::OnTargetPerceptionUpdated(AActor* Actor, FAIStimulus Stimulus)
{
	...
	const auto InCombatInterface = Cast<ICombatInterface>(Actor);
	if (!InCombatInterface) { return; }
